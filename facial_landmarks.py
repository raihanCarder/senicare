...
def data_uri_to_cv2_img(uri):
    # Accept either:
    # 1) A full data URI: "data:image/jpeg;base64,<...>"
    # 2) Raw base64: "<...>"
    if "," in uri:
...
def load_image(image_input: str):
    ...
    - raw base64 string
    - local file path
    Returns a cv2 image (BGR) or None.
    """
    if not image_input:
        return None

    s = image_input.strip()
    if s.startswith("data:") or "," in s:
        return data_uri_to_cv2_img(s)

    if os.path.exists(s):
        return cv2.imread(s)

    # Best-effort: treat as base64 if it isn't a file path.
    try:
        return data_uri_to_cv2_img(s)
    except Exception:
        return None
...
# imageP = args["image"]
# imageP was a hard-coded Windows path in the original project; keep CLI-based usage instead.
imageP = None
...
def resim_analiz(imageP, shape_predictor="shape_predictor_68_face_landmarks.dat"):
    ...
    image = load_image(imageP)
    if image is None:
        durum["error"] = "Could not decode image input"
        return durum
    ...
    if len(corarray) < 55:
        durum["error"] = "Insufficient landmarks detected"
        return durum
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        ...
        help="path to facial landmark predictor .dat",
    )
    ap.add_argument(
        "-i",
        "--image",
        required=True,
        help="image file path OR data URI OR raw base64",
    )
    args = ap.parse_args()

    durum = resim_analiz(args.image, shape_predictor=args.shape_predictor)
